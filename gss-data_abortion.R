...
rm(list = ls())
pacman::p_load(haven, labelled, janitor, gssr, psych, panelr, modelsummary, tidyverse)
...
gss_panel06_long$abscale6 <- 
  psych::alpha(gss_panel06_long[, c(
    "abdefect", "abhlth", "abnomore", "abpoor", "abrape", "absingle")])$scores |> 
  as_vector()
gss_panel06_long$abscale7 <- 
  psych::alpha(gss_panel06_long[, c(
    "abdefect", "abhlth", "abnomore", "abpoor", "abrape", "absingle", "abany")])$scores |> 
  as_vector()
...
gss_panel08_long$abscale6 <- 
  psych::alpha(gss_panel08_long[, c(
    "abdefect", "abhlth", "abnomore", "abpoor", "abrape", "absingle")])$scores |> 
  as_vector()
...
gss_panel10_long$abscale6 <- 
  psych::alpha(gss_panel10_long[, c(
    "abdefect", "abhlth", "abnomore", "abpoor", "abrape", "absingle")])$scores |> 
  as_vector()
...
gss_panel06_long <- 
  gss_panel06_long |> 
  mutate(
    everchild = ifelse(childs == 0, 0, 1), evermarry = ifelse(marital == 5, 0, 1))
...
gss_panel06_long <- gss_panel06_long |> 
  dplyr::select(firstid, wave, abscale6, abscale7, everchild, evermarry, sex,
...
gss <- gss |> 
  ...
  mutate(
    ...
    everchild_2 = ifelse(
      everchild_1 == 1 & everchild_2 == 0, 1, everchild_2),
    # child in wave 3, according to wave 1 and 2
    everchild_3 = ifelse(
      (everchild_1 == 1 | everchild_2 == 1) & everchild_3 == 0, 1, everchild_3),
    # marry in wave 2, according to wave 1
    evermarry_2 = ifelse(
      evermarry_1 == 1 & evermarry_2 == 0, 1, evermarry_2),
    # marry in wave 3, according to wave 1 and 2
    evermarry_3 = ifelse(
      (evermarry_1 == 1 | evermarry_2 == 1) & evermarry_3 == 0, 1, evermarry_3)) |> 
  long_panel(prefix = "_", begin = 1, end = 3, id = "firstid", wave = "wave")

# - part 06: model analysis ------------------------------------------------------------------- #

# female
m1_f <-
  panelr::wbm(abscale7 ~ everchild,
              id = firstid, wave = wave, use.wave = TRUE, wave.factor = TRUE, 
              data = gss |> filter(sex == 2), model = "within")
m2_f <-
  panelr::wbm(abscale7 ~ evermarry,
              id = firstid, wave = wave, use.wave = TRUE, wave.factor = TRUE,
              data = gss |> filter(sex == 2), model = "within")
m3_f <-
  panelr::wbm(abscale7 ~ everchild + evermarry,
              id = firstid, wave = wave, use.wave = TRUE, wave.factor = TRUE,
              data = gss |> filter(sex == 2), model = "within")
...
m1_m <-
  panelr::wbm(abscale7 ~ everchild,
              id = firstid, wave = wave, use.wave = TRUE, wave.factor = TRUE,
              data = gss |> filter(sex == 1), model = "within")
m2_m <-
  panelr::wbm(abscale7 ~ evermarry,
              id = firstid, wave = wave, use.wave = TRUE, wave.factor = TRUE,
              data = gss |> filter(sex == 1), model = "within")
...
saveRDS(m1_f, file = "./abortion_models/m1.RDS")
saveRDS(m2_f, file = "./abortion_models/m2.RDS")
saveRDS(m3_f, file = "./abortion_models/m3.RDS")
saveRDS(m1_m, file = "./abortion_models/m4.RDS")
saveRDS(m2_m, file = "./abortion_models/m5.RDS")
saveRDS(m3_m, file = "./abortion_models/m6.RDS")
...
m4 <-
  panelr::wbm(abscale7 ~ everchild,
              ...
              data = gss, model = "w-b")
m4.int <-
  panelr::wbm(abscale7 ~ everchild | sex | everchild * sex,
              id = firstid, wave = wave, use.wave = TRUE, wave.factor = TRUE, 
              data = gss, model = "w-b")
...
m5 <-
  panelr::wbm(abscale7 ~ evermarry,
              ...
              data = gss, model = "w-b")
m5.int <-
  panelr::wbm(abscale7 ~ evermarry | sex | evermarry * sex,
              id = firstid, wave = wave, use.wave = TRUE, wave.factor = TRUE, 
              data = gss, model = "w-b")
