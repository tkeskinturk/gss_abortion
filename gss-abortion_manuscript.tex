...
Table 1 shows the fixed effects estimates, documenting that there are no substantial effects of first marriage or first birth on abortion attitudes.\footnote{Replication code for the analyses can be found at \url{https://github.com/tkeskinturk/gss_abortion}.} The estimates are imprecise and the effect sizes are highly small, both in female and male samples. The exception is the effect of marriage for male respondents, which is negative and reliably different than 0 at the 95\% level, though the effect size is, once again, quite small.\footnote{In another specification, we estimated within-between models by specifying respondent sex as a cross-level interaction. BIC for the interaction models were lower than the ones without interaction (1425 \textless{} 1445 for Birth and 1449 \textless{} 1464 for Marriage).}
